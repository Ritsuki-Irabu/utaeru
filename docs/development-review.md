...
フロントエンドは、画面表示、ユーザー操作、状態管理、API呼び出しを担当する。バックエンドは、認証、認可、DB操作、Spotify連携、CSV出力を担当する。DBにはMySQLを使用し、ユーザー、曲マスタ、マイリスト、タグを管理する。
...
具体的には、Sanctumによるログイン認証、Spatie Permissionによる`admin` / `user`の権限管理、曲マスタCRUD、マイリストCRUD、CSV出力、Spotify API連携を実装した。
...
ログイン画面、マイリスト画面、曲検索画面、リズム再生コンポーネント、管理者画面を実装した。API通信は`src/api/`に分離し、ログイン状態や曲一覧、マイリストの状態はPiniaで管理した。

これにより、画面コンポーネントは表示と操作に集中でき、API通信や状態管理の責務を分けられた。

## 5. Issue単位での開発プロセス

Issue #1〜#14では、機能を小さな単位に分けて段階的に実装した。
...
```text
管理者が曲名・アーティスト名・BPMを登録
↓
Laravel APIがバリデーションを実行
↓
songsテーブルに保存
↓
ユーザーが曲検索画面を開く
↓
VueがGET /api/songsを呼び出す
↓
Laravelが曲マスタ一覧を取得
↓
API Resourceでレスポンスを整形
↓
Vueが曲名・アーティスト名・BPMを表示
↓
ユーザーがマイリストへ追加
↓
マイリスト画面でリズム再生
...
Vueコンポーネントでは`setInterval`を使って現在の拍を進め、1拍目を強拍、2〜4拍目を弱拍として画面上に表示する。画面遷移などでコンポーネントが破棄されるときは、`onUnmounted`で`clearInterval`を実行し、タイマーが動き続けないようにしている。

当初はSpotify APIからBPMを取得する設計だったが、API利用制限により実運用では管理者がBPMを手入力する形になっている。
...
- AxiosによるAPI通信
- PWA構成の基本
- `setInterval`と`onUnmounted`を使ったリズム再生処理
...
Spotify Web APIについて、当初はBPM取得に利用する想定だったが、実際にはAPIの一部がDeprecatedであったり、利用条件の制限により想定通りに利用できない部分があった。

その結果、管理者が曲名、アーティスト、BPMを直接登録する運用になり、曲追加の手間が増えた。
...
また、実装前にディレクトリ構成をより丁寧に整理しておけば、各ファイルの役割をさらに理解しやすかったと感じた。Laravelでは、`app/Http/Controllers`、`app/Http/Requests`、`app/Http/Resources`、`app/Models`、`app/Services`など、配置場所ごとに責務が分かれている。
...
ウタエルの開発を通して、Laravel APIとVue.js PWAを組み合わせたSPA型アプリの全体像を学ぶことができた。

特に、バックエンドとフロントエンドの責務分離、APIを中心としたデータの流れ、認証・認可・CRUD・状態管理の基本を、Issue単位の実装を通じて理解できたことが大きな成果である。
